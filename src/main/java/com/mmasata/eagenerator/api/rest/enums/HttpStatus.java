...
/**
 * An enum defining a list of HTTP status codes, including names and descriptions.
 */
@Getter
@AllArgsConstructor
public enum HttpStatus {
    ...
    GenericClientError("4XX", "Client error", "Client error"),

    GenericServerError("5XX", "Server error", "Server error"),
    ...
    public static HttpStatus findByCode(String code) {
        return Arrays.stream(HttpStatus.values())
                .filter(httpStatus -> httpStatus.code.equalsIgnoreCase(code))
                .findFirst()
                .orElse(Unknown);
    }
